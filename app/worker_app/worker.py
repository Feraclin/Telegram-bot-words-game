...
class Worker:
        ...
        if upd.message:
            try:
                match upd.message.text:
                    ...
                    case "/reply":
                        await self.tg_client.send_message(
                            chat_id=upd.message.chat.id, text=f"@{upd.message.from_.username} reply", force_reply=True
                        )
                    ...
                    case _ if await self.words_game.select_active_session_by_id(user_id=upd.message.from_.id):
    ...
    async def start_game(self, upd: UpdateObj) -> None:
        if await self.words_game.select_active_session_by_id(upd.message.from_.id):
            ...
            return
        user = await self.words_game.create_user(user_id=upd.message.from_.id, username=upd.message.from_.username)
        await self.words_game.create_game_session(
            user_id=user.id, chat_id=upd.message.chat.id, chat_type=upd.message.chat.type
        )
        await self.tg_client.send_message(chat_id=upd.message.chat.id, text=f"{upd.message.from_.username} let's play")
        if upd.message.chat.type == "private":
            await self.pick_city(
                user_id=upd.message.from_.id, chat_id=upd.message.chat.id, username=upd.message.from_.username
            )
        else:
    ...
    async def pick_city(self, user_id: int, chat_id: int, username: str, letter: str | None = None) -> None:
        game = await self.words_game.select_active_session_by_id(user_id)
        city = await self.words_game.get_city_by_first_letter(letter=letter, game_session_id=game.id)
        if not city:
    ...
    async def check_city(self, user_id: int, chat_id: int, username: str, city_name: str) -> None:
        if city := await self.words_game.get_city_by_name(city_name.strip("/")):
            ...
            if await self.words_game.check_city_in_used(city_id=city.id, game_session_id=game.id):
                await self.tg_client.send_message(chat_id=chat_id, text=f"{username} {city.name}, Был такой город.")
                return
            if game.next_start_letter == city_name[1]:
                await self.words_game.update_game_session(game_id=game.id, next_letter=letter)
                await self.words_game.set_city_to_used(city_id=city.id, game_session_id=game.id)
                await self.tg_client.send_message(
                    chat_id=chat_id, text=f"{username} {city.name} Есть такой город. Мне на {letter}"
                )
                await self.pick_city(user_id=user_id, chat_id=chat_id, username=username, letter=letter)
            else:
                await self.tg_client.send_message(
                    chat_id=chat_id,
            ...
            await self.tg_client.send_message(chat_id=chat_id, text=f"{username} {city_name} Нет такого города")
    ...
    async def chose_your_team(self, upd: UpdateObj) -> None:
        ...
        await self.words_game.create_game_session(
            user_id=upd.message.from_.id, chat_id=upd.message.chat.id, chat_type=upd.message.chat.type
        )
        inline_message = await self.tg_client.send_keyboard(
            upd.message.chat.id, text=f"Будешь играть?", keyboard=keyboard
        )
    ...
    async def add_to_team(self, upd: UpdateObj) -> None:
        game = await self.words_game.select_active_session_by_id(chat_id=upd.callback_query.message.chat.id)
        if game:
            ...
            await self.tg_client.send_callback_alert(
                callback_id=upd.callback_query.id, text=f"{upd.callback_query.from_.username} теперь ты в игре"
            )
    ...
    async def pick_leader(self, game: GameSession, player: int = None):
        ...
        if not team:
            await self.words_game.update_game_session(game_id=game.id, status=False)
            return await self.tg_client.send_message(chat_id=game.chat_id, text="Игорьков больше нет")
    ...
    async def check_word(self, upd: UpdateObj) -> None:
        ...
        if game.next_user_id != upd.message.from_.id:
            await self.tg_client.send_message(
                chat_id=upd.message.chat.id, text=f"{upd.message.from_.first_name} Не твой ход минус жизнь"
            )
            await self.words_game.remove_life_from_player(game_id=game.id, player_id=upd.message.from_.id)
        elif game.next_start_letter and game.next_start_letter.lower() != word[0].lower():
            await self.tg_client.send_message(
                chat_id=upd.message.chat.id,
                text=f"{upd.message.from_.username} " f"Надо слово на букву {game.next_start_letter}",
            )
        elif game.words and word in game.words:
            await self.tg_client.send_message(chat_id=upd.message.chat.id, text=f"Слово {word} уже было")
        else:
    ...
    async def right_word(self, game: GameSession, word: str):
        ...
        await self.words_game.update_game_session(game_id=game.id, next_letter=last_letter, words=game.words)
    ...
    async def bot_looser(self, game_session_id: int) -> None:
        ...
        await self.tg_client.send_message(chat_id=game.chat_id, text=f"Удивительно, я проиграл, опять слово на Ы")
    ...
    async def check_poll(self, upd: UpdateObj) -> None:
        ...
        if yes > no:
            ...
            await self.tg_client.send_message(chat_id=game.chat_id, text=f"{word} - нет такого слова")
